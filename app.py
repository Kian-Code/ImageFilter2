...
def upload_image():
    ...
    if operation_selection == 'gray':
        ...
        file_data = make_sketch(decode_array_to_img)
    else:
...
def make_sketch(decode_array_to_img):
    converted_gray_img = cv2.cvtColor(decode_array_to_img , cv2.COLOR_RGB2GRAY)
    sharpen_gray = cv2.bitwise_not(converted_gray_img)
    blur_img = cv2.GaussianBlur(sharpen_gray , (111,111),0)
    sharpen_blur = cv2.bitwise_not(blur_img)
    final = cv2.divide(converted_gray_img , sharpen_blur , scale = 256.0)
    status , output_image = cv2.imencode('.PNG' , final)
    return output_image
# Ends here
